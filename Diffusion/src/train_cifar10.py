...
def parse_args():
    ...
    parser.add_argument('--log_dir', type=str, default='./log_cifar10', help='Log name [default: log_cifar10]')
    parser.add_argument('--n_workers', type=int, default=1, help='number of workers [default: 1]')
    parser.add_argument('--l_rate', type=float, default=2e-4, help='learning rate [default: 0.0002]')
    parser.add_argument('--g_weight', type=float, default=[1., 3.],
                        help='guidance weights for conditional generation [default: [1., 3.]]')
    ...
    parser.add_argument('--n_samples', type=int, default=2,
                        help='samples to be generated for each class during testing [default: 2]')
    parser.add_argument('--im_height', type=int, default=32, help='image height  [default: 32]')
    parser.add_argument('--im_channels', type=int, default=3, help='input channels [default: 3]')
...
def train_mnist(args):
    ...
    for epoch in range(n_epochs):
        ...
        for y, c in pbar:
            ...
            optimizer.zero_grad()
            # simple scaling of CIFAR-10 dataset to the range -1, +1
        ...
        # save the model
        if epoch % 10 == 0 or epoch == int(n_epochs - 1):
            # --- YOUR CODE HERE ---#
        ...
        if epoch % 10 == 0 or epoch == int(n_epochs - 1):
            ...
            with torch.no_grad():
                ...
                for g_weight_i in g_weight:
                    ...
                    y_sampled, y_sampled_steps = ddpm.sample(n_samples, (channels, height, height), n_classes,
                                                             g_weight=g_weight_i)
                    ...
                    y_sampled_steps = min_max_scale(y_sampled_steps, -1, 1, 0, 1)

                    # save images
                    save_path = os.path.join(log_dir,
                                             'generated_images_epoch{}_guidance_weight{}.png'.format(epoch + 1,
                                                                                                     g_weight_i))
                    plot_image(y_all, n_classes, save_path, inverse=False)

                    # save animated images
                    if epoch % 20 == 0 or epoch == int(n_epochs - 1):
                        plot_animation(y_sampled_steps, n_samples, n_classes, log_dir, epoch, g_weight_i, rgb=True)
